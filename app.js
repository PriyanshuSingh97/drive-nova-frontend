...
function setupLoginModalEvents() {
	...
	// Unified OAuth button setup
	document.getElementById('google-login-login')?.addEventListener('click', function() {
		handleAuthRedirect(this, 'google');
	});
	document.getElementById('google-login-register')?.addEventListener('click', function() {
		handleAuthRedirect(this, 'google');
	});
	document.getElementById('github-login-login')?.addEventListener('click', function() {
		handleAuthRedirect(this, 'github');
	});
	document.getElementById('github-login-register')?.addEventListener('click', function() {
		handleAuthRedirect(this, 'github');
	});
}

/**
 * Handles the OAuth redirection process.
 * @param {HTMLElement} button The button element that was clicked.
 * @param {string} provider The OAuth provider ('google' or 'github').
 */
function handleAuthRedirect(button, provider) {
	// Prevent multiple clicks if already disabled
	if (button.classList.contains('disabled')) return;

	button.classList.add('disabled');
	button.innerHTML = `Connecting to ${provider.charAt(0).toUpperCase() + provider.slice(1)}... ${createButtonSpinner()}`;
...
/**
 * Resets OAuth buttons to their default, clickable state.
 * This function targets the actual button IDs in your HTML.
 */
function resetOAuthButtons() {
	// Google Login buttons - using the actual IDs from your HTML
	const googleLoginLogin = document.getElementById('google-login-login');
	if (googleLoginLogin) {
		googleLoginLogin.classList.remove('disabled');
		googleLoginLogin.innerHTML = `<img src="https://res.cloudinary.com/dtvyar9as/image/upload/v1756950611/google-logo_qf8vsc.png" alt="Google"> <span>Continue with Google</span>`;
	}

	const googleLoginRegister = document.getElementById('google-login-register');
	if (googleLoginRegister) {
		googleLoginRegister.classList.remove('disabled');
		googleLoginRegister.innerHTML = `<img src="https://res.cloudinary.com/dtvyar9as/image/upload/v1756950611/google-logo_qf8vsc.png" alt="Google"> <span>Continue with Google</span>`;
	}

	// GitHub Login buttons - using the actual IDs from your HTML
	const githubLoginLogin = document.getElementById('github-login-login');
	if (githubLoginLogin) {
		githubLoginLogin.classList.remove('disabled');
		githubLoginLogin.innerHTML = `<img src="https://res.cloudinary.com/dtvyar9as/image/upload/v1756950613/github-logo_l0mgoo.png" alt="GitHub"> <span>Continue with GitHub</span>`;
	}

	const githubLoginRegister = document.getElementById('github-login-register');
	if (githubLoginRegister) {
		githubLoginRegister.classList.remove('disabled');
		githubLoginRegister.innerHTML = `<img src="https://res.cloudinary.com/dtvyar9as/image/upload/v1756950613/github-logo_l0mgoo.png" alt="GitHub"> <span>Continue with GitHub</span>`;
	}
}
...
function openLoginModal() {
	...
	if (loginModal) {
		resetOAuthButtons(); // <-- Reset buttons every time the modal is opened
...
function openBookingModal(name, price) {
	const carToBook = currentSelectedCar || {
		name,
